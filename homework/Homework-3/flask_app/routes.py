...
def processlogin():
    ...
    print(db.authenticate(request.form['email'],request.form['password']))

    if db.authenticate(request.form['email'],request.form['password']) == False:
        print(json.dumps({'success':2}))
...
def leave(message):
    leave_room('main')
    emit('status', {'msg': getUser() + ' has leave the room.', 'style': 'width: 100%;color:blue;text-align: right'}, room='main')


@socketio.on('sentMessage', namespace='/chat')
def sentMessage(message):
    print(message)
    print(getUser())
    if getUser() == "owner@email.com":
        emit('messageStatus', {'msg': message['msg'], 'style': 'width: 100%;color:blue;text-align: right'}, room='main')
    else:
        emit('messageStatus', {'msg': message['msg'], 'style': 'width: 100%;color:grey;text-align: left'}, room='main')
...
def resume():
	...
	return render_template('newResume.html', user=getUser(), resume_data = resume_data)
...
def processfeedback():
    ...
    return render_template('processfeedback.html', result = result, user=getUser())
